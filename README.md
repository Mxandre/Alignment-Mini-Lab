...
When $\beta$ is reduced from $0.3$ to $0.2$, the token-level KL divergence surges by $0.3$ nats. According to the information-theoretic decomposition $D_{KL}(P \parallel R) = H(P, R) - H(P)$, and given that DPO training tends to reduce the model’s own entropy $H(P)$ (to maximize the reward), the surge in KL implies a more dramatic increase in cross-entropy $H(P, R)$. Intuitively, the model with $\beta=0.2$ generates an extremely narrow and pathological representation pattern in order to align with the preferences. This pattern appears extremely unfamiliar and ungrammatical to the reference baseline (SFT), and its equivalent cross-perplexity has increased by over 35%. This provides a perfect mathematical explanation for why, under this hyperparameter, the model exhibits obvious out-of-distribution (OOD) hallucinations such as the mixing of Chinese and English.
